...
Abstract:
	This project focuses on monitoring room temperature and controlling a cooler using the STM32L4 microcontroller and LM35 temperature sensor.
  The LM35 provides an Analog voltage proportional to temperature, which is converted into a digital value using the STM32L4’s ADC. 
  The processed temperature data is sent to a PC through UART communication, allowing real-time monitoring on a serial terminal. 
  The system supports both automatic and manual control. In automatic mode, the cooler turns ON when the temperature exceeds 28°C and OFF 
  when it drops below the threshold. In manual mode, the user can send commands such as ON, OFF, or AUTO through the serial window to 
  control the cooler remotely. This project demonstrates a simple and efficient embedded solution suitable for home automation,
  environmental monitoring, and basic IoT applications.
Required Components:
